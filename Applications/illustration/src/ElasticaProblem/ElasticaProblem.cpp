...
void ElasticaProblem::testElasticaInput()
{
    ...
    createAndSaveDS(A,B,piOverFour,piOverFour,outputFolder + "/elasticaDS-0.eps");
    createAndSaveDS(A,B,piOverFour, threePiOverFour,outputFolder + "/elasticaDS-1.eps");
    createAndSaveDS(A,B,threePiOverFour, piOverFour,outputFolder + "/elasticaDS-2.eps");
    createAndSaveDS(A,B,threePiOverFour, threePiOverFour,outputFolder + "/elasticaDS-3.eps");

}

void ElasticaProblem::example(const std::string& outputFolder)
{
    Point A(0,0);
    Point B(100,0);
...
//    ElasticaInput ei(A,B,piOverFour, threePiOverFour);
    ...
    for(int i=0;i<iterations;++i)
    {
        ...
        ODRModel ODR = createODR(outputFolder,
                                 ei,
                                 inputDS,
                                 ID.optMode,
                                 ID.appMode,
        ...
        SCaBOliC::Core::Display::DisplayODR(ODR,outputFolder + "/ODR.eps");
        ...
        board.saveSVG(outputFolder + "/solution.svg",200,200,10);
        ...
        SCaBOliC::Core::Display::DisplayModifiedBoundary(ODR,solution.outputDS,outputFolder + "/modified-boundary.eps");
    }
...
{
    if(iteration%2==0) return IterationData(OptimizationMode::OM_CorrectConvexities,ApplicationMode::AM_AroundBoundary);
    else return IterationData(OptimizationMode::OM_CorrectConcavities,ApplicationMode::AM_AroundBoundary);
}
...
ElasticaProblem::ODRModel ElasticaProblem::createODR(const std::string& outputFolder,
                                                     const ElasticaInput& ei,
                                                     ...
                                                     unsigned int radius)
{
    ...
    optRegionDS = optRegion(outputFolder,ei,inputDS,optMode);
...
//    if(optMode==OptimizationMode::OM_CorrectConcavities)
//    {
...
ElasticaProblem::DigitalSet ElasticaProblem::optRegion(const std::string& outputFolder,
                                                       const ElasticaInput &ei,
                                                       const DigitalSet& inputDS,
                                                       const OptimizationMode& optMode)
{
    ...
    board.saveEPS(outputFolder + "/segCurve.eps");

    DigitalSet segCurveDS(inputDS.domain());
    if(optMode==OptimizationMode::OM_CorrectConvexities)
    ...
    board.saveEPS(outputFolder + "/segCurveDS.eps");
