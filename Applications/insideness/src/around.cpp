...
namespace Around
{
    ...
    EnergyTerm createEnergyTerm(const PixelIndexMap& vm, const DigitalSet& appRegion, const DigitalSet& optRegion, const DigitalSet& frgRegion, const DigitalSet& bkgRegion)
    {
        ...
        for(auto it=appRegion.begin();it!=appRegion.end();++it)
        {
            ...
            int s = FInters.size() - BInters.size();

            for(auto jt=OInters.begin();jt!=OInters.end();++jt)
            {
                Index xj = vm.at(*jt);

                term.od.localUTM(1,xj) -= (int) FInters.size();
                term.od.localUTM(0,xj) -= (int) BInters.size();

                for(auto kt=OInters.begin();kt!=OInters.end();++kt)
                {
                    Index xk = vm.at(*kt);
                    if(xk!=xj)
                    {
                        IndexPair ip = term.od.makePair(xk,xj);
    ...
    DigitalSet optRegion(const DigitalSet& ds, bool shrink)
    {
        ...
        if(shrink)
        {
            DIPaCUS::Misc::digitalBoundary<DIPaCUS::Neighborhood::FourNeighborhoodPredicate>(dsOut,ds,4);
        }
        else
        {
    ...
    DigitalSet evolve(const DigitalSet& ds)
    {
        ...
        DigitalSet appRegion = FSet;
        appRegion.insert(BSet.begin(),BSet.end());

        PixelIndexMap  vm = createVarMap(OSet);
        EnergyTerm term = createEnergyTerm(vm,appRegion,OSet,FSet,BSet);
